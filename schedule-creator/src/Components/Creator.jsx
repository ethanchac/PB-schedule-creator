...
function Creator() {
  ...
  // Toggle staff availability in the main view
  const toggleAvailability = (id) => {
    setStaff(staff.map(person => 
      person.id === id ? { ...person, available: !person.available } : person
    ));
  };
      ...
      <div className="staff-list">
        {staff.map(person => (
          <div key={person.id} className="staff-item">
            ...
            <button 
              className={`availability-button ${person.available ? 'available' : ''}`}
              onClick={() => toggleAvailability(person.id)}
            >
              Availability
            </button>
          </div>
